...
agent_communication:
  ...
  - agent: "main"
    message: "Added deployment-safe fallback for roadmap generation: use EMERGENT_LLM_KEY first, otherwise GEMINI_API_KEY with direct Gemini 2.5 Pro REST API. Please backend-test that EMERGENT path still works and note that GEMINI_API_KEY fallback is implemented but cannot be fully runtime-tested without a live Gemini key in env."
