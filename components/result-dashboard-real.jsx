...
const LoadingView = ({ analyzing }) => (
  <div className="flex min-h-[70vh] flex-col items-center justify-center p-8 text-center">
    ...
    <p className="mt-2 text-slate-500 max-w-md">
      {analyzing
        ? 'Our AI is analyzing 60 data points. This takes about 30 seconds — please do not refresh.'
...
const ResultDashboardReal = ({ assessmentId, onReady, isPdfMode }) => {
  ...
  useEffect(() => {
    const loadResult = async () => {
      ...
      try {
        ...
        if (response.status === 402) { setLocked(true); setLoading(false); return }
        ...
        if (hasRealAiAnalysis(currentAssessment?.ai_analysis)) {
          ...
          let roadmapData = {}
          try {
            roadmapData = JSON.parse(await roadmapResponse.text())
          } catch {
            throw new Error("The AI took too long. Please click 'Retake' to resume.")
          }
    ...
    return (
      <div className="container mx-auto py-20 text-center">
        <Card className="mx-auto max-w-lg p-10 shadow-xl">
          ...
          <p className="mt-4 text-slate-600">Complete your one-time contribution to access your full 5-year vision and PDF download.</p>
  ...
  // ── PDF mode compact spacing values ──
  const sp = isPdfMode
    ? { section: 'mb-3', card: 'p-3', text: 'text-sm', gap: 'gap-3' }
    : { section: 'mb-8', card: 'p-6', text: 'text-lg', gap: 'gap-6' }

  return (
    <main className={isPdfMode ? 'h-max bg-white' : 'min-h-screen bg-slate-50 py-8'}>

      {/* FIX 4: Enhanced PDF global styles */}
      {isPdfMode && (
      ...
      <div className={`container mx-auto ${isPdfMode ? 'px-0 max-w-none' : 'space-y-8 px-4 sm:px-6 lg:px-8'}`}>
        ...
        {/* ── HERO BANNER ── */}
        {/* FIX 3: Tighter PDF padding */}
        <section className={`avoid-break bg-[#0A2351] text-white
          ${isPdfMode
            ? 'rounded-xl p-4 mb-3'
            : 'rounded-[2rem] p-8 sm:p-12 relative overflow-hidden mb-8 shadow-2xl shadow-[#0A2351]/20'
          }`}>
          <div className="relative z-10">
            <div className={`inline-flex items-center gap-2 rounded-full bg-white/10 px-4 py-1.5 text-xs font-bold uppercase tracking-widest text-[#F57D14] ${isPdfMode ? 'mb-2' : 'mb-4'}`}>
              <Sparkles className="h-3 w-3" /> Real-Time AI Analysis
            </div>
            <h1 className={`font-extrabold tracking-tight text-white ${isPdfMode ? 'text-2xl' : 'text-4xl sm:text-5xl'}`}>
              {studentName}, you are a{' '}
              <span className="text-[#F57D14]">{analysis.user_archetype}</span>
            </h1>
            {/* FIX 3: tighter mt in PDF */}
            <p className={`text-white/70 leading-relaxed ${isPdfMode ? 'mt-2 text-sm' : 'mt-6 text-lg'}`}>
        ...
        <section className={`avoid-break ${sp.section}`}>
          <SectionHeading icon={BrainCircuit} title="Strategic Executive Summary" subtitle="How SARATHI interprets your unique behavioral fingerprint." isPdfMode={isPdfMode} />
          <Card className="border-0 shadow-sm">
            {/* FIX 3: Tighter padding + smaller text in PDF */}
            <CardContent className={`text-slate-700 leading-relaxed ${isPdfMode ? 'p-4 text-sm space-y-3' : 'p-8 text-lg space-y-4'}`}>
        ...
        {/* ── RADAR + DNA side by side (or stacked in PDF) ── */}
          ...
          <section className={`avoid-break ${sp.section}`}>
            ...
            <Card className="border-0 bg-[#0A2351]/5 shadow-none">
              <CardContent className={isPdfMode ? 'p-3' : 'p-4'}>
                <div className={isPdfMode ? 'h-[200px] w-full' : 'h-[260px] w-full'}>
        ...
        {/* ── CAREER MATCHES ── */}
        <section className={`avoid-break ${sp.section}`}>
          <SectionHeading icon={Target} title="Recommended Career Paths" subtitle="Matched to your specific psychometric scores." isPdfMode={isPdfMode} />
          <div className={isPdfMode ? 'block space-y-3' : 'grid gap-6 md:grid-cols-3'}>
            {(analysis.top_career_matches || []).map((match, i) => (
              <Card key={i} className={`border-0 border-l-4 border-l-[#F57D14] ${isPdfMode ? 'shadow-none border border-slate-200 border-l-4' : 'shadow-sm hover:shadow-md transition-all'}`}>
                <CardContent className={isPdfMode ? 'p-3' : 'p-6'}>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-1">Prime Match</p>
        ...
        {blindSpots.length > 0 && (
          <section className={`avoid-break ${sp.section}`}>
            <SectionHeading icon={Lightbulb} title="Growth Warnings" subtitle="Specific risks identified in your psychometric profile." isPdfMode={isPdfMode} />
        ...
        {/* FIX 1: solid bg in PDF mode instead of gradient */}
        {immediateAction?.next_30_days && (
          <section className={`avoid-break ${sp.section}`}>
            <SectionHeading icon={Timer} title="Immediate Action Plan" isPdfMode={isPdfMode} />
            <Card className={`border-0 text-white ${isPdfMode ? 'bg-[#0A5C44] shadow-none' : 'shadow-lg bg-gradient-to-r from-emerald-600 to-teal-800'}`}>
              <CardContent className={isPdfMode ? 'p-4' : 'p-8'}>
                <div className="space-y-4">
                  <div>
                    <p className={`text-xs font-bold uppercase tracking-widest mb-1 ${isPdfMode ? 'text-white/80' : 'text-emerald-200'}`}>Next 30 Days</p>
                    <p className={`font-bold ${isPdfMode ? 'text-base' : 'text-lg'}`}>{immediateAction.next_30_days}</p>
                  </div>
                  {immediateAction.next_90_days && (
                    <div>
                      <p className={`text-xs font-bold uppercase tracking-widest mb-1 ${isPdfMode ? 'text-white/80' : 'text-emerald-200'}`}>Next 90 Days</p>
        ...
        {analysis.india_vs_abroad_guidance && (
          <section className={`avoid-break ${sp.section}`}>
            <SectionHeading icon={Globe} title="India vs Abroad — Your Path" subtitle="Based on your Q60 response and global ambition score." isPdfMode={isPdfMode} />
            <Card className="border-0 bg-blue-50/50 border border-blue-100">
              <CardContent className={isPdfMode ? 'p-3' : 'p-6'}>
        ...
        {/* ── 5-YEAR ROADMAP ── */}
        {/* FIX 2: shadow-none + border in PDF, tighter padding, explicit pageBreakInside */}
        <section className={isPdfMode ? 'pt-2' : 'mt-4'}>
          <SectionHeading icon={TrendingUp} title="Your 5-Year Career Transformation" subtitle="A milestone-by-milestone plan mapped to your profile." isPdfMode={isPdfMode} />
          <div className={isPdfMode ? 'block space-y-3' : 'grid gap-6 lg:grid-cols-3'}>
            {roadmapSteps.map((step, i) => (
              <div key={i} className="avoid-break" style={{ pageBreakInside: 'avoid' }}>
                <Card className={`border-0 bg-white overflow-hidden ${isPdfMode ? 'shadow-none border border-slate-200' : 'shadow-lg'}`}>
                  <div className={`h-1.5 w-full ${step.color}`} />
                  <CardHeader className={isPdfMode ? 'p-3 pb-1' : ''}>
                    <div className="flex items-center gap-3">
                      <div className={`flex h-9 w-9 items-center justify-center rounded-xl text-white shrink-0 ${step.color}`}>
                        <step.icon className="h-4 w-4" />
                      </div>
                      <div>
                        <p className="text-xs font-bold text-slate-400 uppercase tracking-tight">{step.label}</p>
                        <p className={`font-bold text-[#0A2351] ${isPdfMode ? 'text-base' : 'text-lg'}`}>{step.title}</p>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className={isPdfMode ? 'p-3 pt-0' : ''}>
                    <p
                      className="text-sm leading-relaxed text-slate-600"
                      style={isPdfMode ? { pageBreakInside: 'avoid', orphans: 3, widows: 3 } : {}}
                    >
                      {step.data}
                    </p>
                  </CardContent>
